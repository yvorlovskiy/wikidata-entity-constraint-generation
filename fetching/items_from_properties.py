"""
RETRIEVES MOST COMMON ITEMS WITH A GIVEN PROPERTY
For example: all properties related to entities that played 'quarterback' on a football team (corresponding to P413 and a value of Q622747)

To run:
python3.6 fetch_with_item_and_value.py --data $DATA --out_dir $OUT
"""
...
def filtering_func(property_id, filename):
    ...
    try:
        for item in jsonl_generator(filename):
            ...
            if item.get('property_id') == property_id:
                filtered[item.get('value')].add(item.get('qid'))
...
def main():
    ...
    for output in tqdm(
            pool.imap_unordered(
                partial(filtering_func, args.property), table_files, chunksize=1),
    ...
    entity_counts = Counter({k: len(v) for k, v in merged_filtered.items()})
    top_20_entities = entity_counts.most_common(20)

    print(f"Top 20 entities by count:")
